...
load_messages() {
  ...
  # Load messages from JSON file
  MSG_WELCOME=$(jq -r ".${LANGUAGE}.MSG_WELCOME // \"Welcome to Shadowsocks setup\"" "$LANG_FILE")
  MSG_USAGE=$(jq -r ".$LANGUAGE.MSG_USAGE // \"Usage: vpn [command]\"" $LANG_FILE)
  MSG_COMMANDS=$(jq -r ".$LANGUAGE.MSG_COMMANDS // \"Commands:\"" $LANG_FILE)
  MSG_SETUP=$(jq -r ".$LANGUAGE.MSG_SETUP // \"Install and configure the Shadowsocks container\"" $LANG_FILE)
  MSG_START=$(jq -r ".$LANGUAGE.MSG_START // \"Start the Shadowsocks container\"" $LANG_FILE)
  MSG_STOP=$(jq -r ".$LANGUAGE.MSG_STOP // \"Stop the Shadowsocks container\"" $LANG_FILE)
  MSG_RESTART=$(jq -r ".$LANGUAGE.MSG_RESTART // \"Restart the Shadowsocks container\"" $LANG_FILE)
  MSG_UNINSTALL=$(jq -r ".$LANGUAGE.MSG_UNINSTALL // \"Remove the Shadowsocks container\"" $LANG_FILE)
  MSG_HELP=$(jq -r ".$LANGUAGE.MSG_HELP // \"Show this help message\"" $LANG_FILE)
  MSG_DOCKER_NOT_INSTALLED=$(jq -r ".$LANGUAGE.MSG_DOCKER_NOT_INSTALLED // \"Docker is not installed. Installing Docker...\"" $LANG_FILE)
  MSG_UNSUPPORTED_PM=$(jq -r ".$LANGUAGE.MSG_UNSUPPORTED_PM // \"Unsupported package manager. Please install Docker manually.\"" $LANG_FILE)
  MSG_DOCKER_INSTALLED=$(jq -r ".$LANGUAGE.MSG_DOCKER_INSTALLED // \"Docker is installed.\"" $LANG_FILE)
  MSG_GENERATED_PASSWORD_PROCESS=$(jq -r ".$LANGUAGE.MSG_GENERATED_PASSWORD_PROCESS // \"Please enter a password to generate. If left blank, a random password will be generated.\"" $LANG_FILE)
  MSG_GENERATED_PASSWORD=$(jq -r ".$LANGUAGE.MSG_GENERATED_PASSWORD // \"Enter the password:\"" $LANG_FILE)
  MSG_TZ_CHANGE_PROCESS=$(jq -r ".$LANGUAGE.MSG_TZ_CHANGE_PROCESS // \"Enter the desired timezone (e.g., UTC+3). If left blank, UTC+3 will be used as the default.\"" $LANG_FILE)
  MSG_TZ_CHANGE=$(jq -r ".$LANGUAGE.MSG_TZ_CHANGE // \"Enter the desired timezone (e.g., UTC+3) [optional]:\"" $LANG_FILE)
